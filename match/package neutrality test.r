...
if(is.positive.definite(S1)==FALSE){S1=mdf1(S1,tol=1e-6,tol1=0.001);if(prints){print("S1 transformed to positive definite")}}
if(is.positive.definite(S2)==FALSE){S2=mdf1(S2,tol=1e-6,tol1=0.001);if(prints){print("S2 transformed to positive definite")}}
...
nn=sum(ni);ai=ni/nn;So=S[[1]];p=dim(So)[1];k=length(S);
 for(i in 1:k)
  {if(is.positive.definite(S[[i]])==FALSE){S[[i]]=mdf1(S[[i]],tol=1e-6,tol1=0.001);if(prints){print(paste("S[[",i,"]] transformed to positive definite"))}}}
