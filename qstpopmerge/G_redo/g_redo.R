...
eigen(nGb)
eigen(nGb)$values/sum(eigen(nGb)$values)
cumsum(eigen(nGb)$values/sum(eigen(nGb)$values))
...
pdf("G_eigens1_4.pdf", width=8, height=6)
par(mar=c(2.5,3,3,3),oma = c(1, 1, 4, 1))
layout(matrix(c(1, 2, 3, 4),2,2, byrow=TRUE),heights=c(1, 1))
barplot(eigvw[,1],names=name, main=expression("G"[W]*" PC1: 39.3%"), col=color, cex.names=.7)
barplot(eigvw[,2],names=name, main=expression("G"[W]*" PC2: 22.2%"), col=color, cex.names=.7)
barplot(eigvw[,3],names=name, main=expression("G"[W]*" PC3: 15.3%"), col=color, cex.names=.7)
barplot(eigvw[,4],names=name, main=expression("G"[W]*" PC4: 9.2%"), col=color, cex.names=.7)
mtext("PC trait loadings for G-within", outer = TRUE, cex = 1.5)
barplot(eigvb[,1],names=name, main=expression("G"[B]*" PC1: 87.5%"), col=color, cex.names=.7)
barplot(eigvb[,2],names=name, main=expression("G"[B]*" PC2: 10.5%"), col=color, cex.names=.7)
barplot(eigvb[,3],names=name, main=expression("G"[B]*" PC3: 1.6%"), col=color, cex.names=.7)
barplot(eigvb[,4],names=name, main=expression("G"[B]*" PC4: 0.25%"), col=color, cex.names=.7)
mtext("PC trait loadings for G-between", outer = TRUE, cex = 1.5)
dev.off()
...
pdf("G_pca.pdf", width=12, height=6)
par(mfrow=c(1,2))
x <- Gw
...
biplot(pca.x, main="G-within biplot")

y <- nGb
pca.y <- princomp(y,scale=TRUE, center=TRUE)
summary(pca.y)
names(pca.y)
# screeplot(pca.y, type="barplot")
biplot(pca.y, main="G-between biplot")
dev.off()
